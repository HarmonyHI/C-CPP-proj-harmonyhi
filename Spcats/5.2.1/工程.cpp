...
int mklvr(type)
{
	...
	while (ctrldb[119] < ctrldb[121])
	{
		...
		if (((touchlocate > 4) || (touchlocate < 1)) && (highsexmode == 0))
		{
		    if (sexlogger!=0)
		    {
			senread(75);
			...
			if (q == 12)
			{
				...
				while ((ctrldb[6] > 0) && (q == 12))
				{
					...
					if ((ctrldb[6] > 200) && (ctrldb[3] < 0))
					{
						...
						ctrldb[119] = 0;
						system("pause");
						printf("房事前生命%d", ctrldb[1]);
						ctrldb[1] = ctrldb[1] - ctrldb[110] * (16 - ctrldb[7]);
						printf("房事后生命%d", ctrldb[1]);
					}
...
mklvr(4);
						...
						printf
							("年龄%d\n生命%d\n饱腹感%d\n性%d\n清洁%d\n情绪%d\n经验值%d\n总资产%d\n乳头敏感度%d\n阴蒂敏感度%d\n舌敏感度%d\n阴道敏感度%d\n高潮上限%d\n",
							 ctrldb[7], ctrldb[1], ctrldb[2], ctrldb[3], ctrldb[4], ctrldb[5],
							 ctrldb[6], ctrldb[12], ctrldb[115], ctrldb[116], ctrldb[118],
							 ctrldb[120], ctrldb[121]);
						printf
							("1 去床上\n2 去浴室\n3 去超市\n4 去工作\n5 返回主菜单\n6 去医院\n7 去厨房\n其他任意键跳过选择\n");
