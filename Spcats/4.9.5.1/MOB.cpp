...
{
	{
		...
		system("mkdir /sdcard/ProgramFiles");
		char stxt[NTXTREADY][100], *ptxt, *qtxt = "/sdcard/ProgramFiles/CTDATA.txt";
		FILE *fptxt;
		if ((fptxt = fopen("/sdcard/ProgramFiles/CTDATA.txt", "w+")) == NULL)
		{
		...
		while (extraier < 100)
		{
			...
			while (iwt < 0 && fgets(school[iwt], 101, fptxt) != NULL)
			{
				ptxt = strchr(school[iwt], '\n');
				if (ptxt)
					*ptxt = '\0';	/* 消除每行最后的回车符 */
				iwt++;
			}
			strcpy(stxt[iwt++], school);	/* 插入到第一行 */
...
int Datareader(sewhat)
{
	...
	{
		school[sewhatdata - 1] = wordread(sewhatdata);
...
int wordread(whichLine)
{
	...
	char strLine[1024];
	if ((datafile = fopen("/sdcard/ProgramFiles/CTDATA.txt", "r")) == NULL)
	{
		printf("error!");
		return NULL;
	}
...
int main(systemlink)
{
	...
	int freecho, petheart, petmood, petsex, pethungry, petclean, pettype, girlage, matter,
		...
		sexaac, sexaad, touchcounter, sextop, sexnow, sextoolhistory, touchlocate;
	srand(time(0));
	while (beginning <= 100)
	{
		school[xxx] = 0;
	...
	while (menucir == 2)
	{
			   ...
			   || (q == 1123) || (q == 12))
		{
			if ((q == 15) || (autotest == 1))
			{
					...
					system("cls");
					int quizstrength, quiztimea, quiztimeb, deltatime, ingstrength = 1, calcore =1,ingchecker=0,ingbeep=0,printbugfixaa=0,printbugfixab=0,printbugfixac=0,printbugfixad=0;
					printf("设备运算性能评估v1.0\n\n\n\n");
